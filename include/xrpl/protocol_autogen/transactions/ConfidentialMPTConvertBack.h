 ...
 *
 * Type: ttMPTOKEN_CONVERT_BACK (87)
...
{
public:
    static constexpr xrpl::TxType txType = ttMPTOKEN_CONVERT_BACK;
...
public:
    ...
    ConfidentialMPTConvertBackBuilder(SF_ACCOUNT::type::value_type account,
                     std::decay_t<typename SF_UINT192::type::value_type> const& mPTokenIssuanceID,                     std::decay_t<typename SF_UINT64::type::value_type> const& mPTAmount,                     std::decay_t<typename SF_VL::type::value_type> const& holderEncryptedAmount,                     std::decay_t<typename SF_VL::type::value_type> const& issuerEncryptedAmount,                     std::decay_t<typename SF_VL::type::value_type> const& blindingFactor,                     std::decay_t<typename SF_VL::type::value_type> const& balanceCommitment,                     std::decay_t<typename SF_VL::type::value_type> const& zKProof,                    std::optional<SF_UINT32::type::value_type> sequence = std::nullopt,
                    std::optional<SF_AMOUNT::type::value_type> fee = std::nullopt
)
        : TransactionBuilderBase<ConfidentialMPTConvertBackBuilder>(ttMPTOKEN_CONVERT_BACK, account, sequence, fee)
    {
    ...
    {
        if (tx->getTxnType() != ttMPTOKEN_CONVERT_BACK)
        {
