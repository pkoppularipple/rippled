...
\begin{protocol}[$\pi_{\mathrm{eq}}$: constant-size shared-$r$ equality]
  ...
  \begin{enumerate}
    ...
    \item \textbf{Challenge (Fiat--Shamir).}
          \[
            c = H\bigl(C
            \| \Delta D_2 \| \cdots \| \Delta D_n
            \| \Delta P_2 \| \cdots \| \Delta P_n
            \| R_G \| R_2 \| \cdots \| R_n\bigr).
          \]
          The public bases $\Delta P_i$ are included so that
          the challenge binds to the full public statement,
          not just to the commitments $\Delta D_i$ and
          announcements $R_i$.  Omitting them would let a
          malicious prover choose $\Delta P_i = s^{-1}(R_i + c\,\Delta D_i)$
          for arbitrary $R_i$ after seeing $c$, making the
          verifier's reconstruction equation hold for a
          statement different from the one the prover
          committed to.
