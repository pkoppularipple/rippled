 ...
 *
 * Type: ttMPTOKEN_CONFIDENTIAL_SEND (88)
...
{
public:
    static constexpr xrpl::TxType txType = ttMPTOKEN_CONFIDENTIAL_SEND;
...
public:
    ...
    ConfidentialMPTSendBuilder(SF_ACCOUNT::type::value_type account,
                     std::decay_t<typename SF_UINT192::type::value_type> const& mPTokenIssuanceID,                     std::decay_t<typename SF_ACCOUNT::type::value_type> const& destination,                     std::decay_t<typename SF_VL::type::value_type> const& senderEncryptedAmount,                     std::decay_t<typename SF_VL::type::value_type> const& destinationEncryptedAmount,                     std::decay_t<typename SF_VL::type::value_type> const& issuerEncryptedAmount,                     std::decay_t<typename SF_VL::type::value_type> const& amountCommitment,                     std::decay_t<typename SF_VL::type::value_type> const& balanceCommitment,                     std::decay_t<typename SF_VL::type::value_type> const& zKProof,                    std::optional<SF_UINT32::type::value_type> sequence = std::nullopt,
                    std::optional<SF_AMOUNT::type::value_type> fee = std::nullopt
)
        : TransactionBuilderBase<ConfidentialMPTSendBuilder>(ttMPTOKEN_CONFIDENTIAL_SEND, account, sequence, fee)
    {
    ...
    {
        if (tx->getTxnType() != ttMPTOKEN_CONFIDENTIAL_SEND)
        {
