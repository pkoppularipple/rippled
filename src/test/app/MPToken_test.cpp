...
class MPToken_test : public beast::unit_test::Suite
{
    ...
    testConfidentialMPT(FeatureBitset features)
    {
        ...
        // XLS-0096: an issuer registers EC-ElGamal encryption keys on a
        // confidential issuance via MPTokenIssuanceSet.
        Account const bob("bob");
        std::string const issuerKey(33, '\x02');
        std::string const auditorKey(33, '\x03');

        // Key registration is rejected unless the ConfidentialMPT amendment is
        // enabled.
        {
            Env env{*this, features - featureConfidentialMPT};
            MPTTester mptAlice(env, alice);
            ...
            mptAlice.set(
                {.account = alice, .issuerEncryptionKey = issuerKey, .err = temDISABLED});
        }

        // Key registration is rejected on an issuance that does not have the
        // confidential-amount capability enabled.
        {
            Env env{*this, features};
            MPTTester mptAlice(env, alice);
        ...
        // A non-issuer cannot register encryption keys.
        {
            Env env{*this, features};
        ...
        // The issuer registers both keys on a confidential issuance; the keys
        // are stored on the MPTokenIssuance ledger entry.
        {
            Env env{*this, features};
            MPTTester mptAlice(env, alice);
            mptAlice.create(
