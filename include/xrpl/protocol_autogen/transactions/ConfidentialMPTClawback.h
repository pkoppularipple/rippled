 ...
 *
 * Type: ttMPTOKEN_CLAWBACK_CONFIDENTIAL (89)
 * Delegable: Delegation::NotDelegable
 * Amendment: featureConfidentialMPT
 * Privileges: NoPriv
 *
...
{
public:
    static constexpr xrpl::TxType txType = ttMPTOKEN_CLAWBACK_CONFIDENTIAL;
...
public:
    ...
    ConfidentialMPTClawbackBuilder(SF_ACCOUNT::type::value_type account,
                     std::decay_t<typename SF_UINT192::type::value_type> const& mPTokenIssuanceID,                     std::decay_t<typename SF_ACCOUNT::type::value_type> const& holder,                     std::decay_t<typename SF_UINT64::type::value_type> const& mPTAmount,                     std::decay_t<typename SF_VL::type::value_type> const& blindingFactor,                     std::decay_t<typename SF_VL::type::value_type> const& zKProof,                    std::optional<SF_UINT32::type::value_type> sequence = std::nullopt,
                    std::optional<SF_AMOUNT::type::value_type> fee = std::nullopt
)
        : TransactionBuilderBase<ConfidentialMPTClawbackBuilder>(ttMPTOKEN_CLAWBACK_CONFIDENTIAL, account, sequence, fee)
    {
    ...
    {
        if (tx->getTxnType() != ttMPTOKEN_CLAWBACK_CONFIDENTIAL)
        {
     ...
     */
    ConfidentialMPTClawbackBuilder&
    setBlindingFactor(std::decay_t<typename SF_VL::type::value_type> const& value)
    {
